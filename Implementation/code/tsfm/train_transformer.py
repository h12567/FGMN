...
def getInput13(vertex, msp): #transformer without positional encoding
    src = torch.LongTensor([[padding_idx] * k] * len(vertex))  # [batch, k]
    dec_input = torch.LongTensor([[[0] *4]*edge_num] * len(vertex))  # [batch, edge_num, 4]
    for b in range(len(vertex)):
        idx = 0
        for j in range(1, len(msp[b]) - 1):
            if msp[b][j - 1] < msp[b][j] and msp[b][j + 1] < msp[b][j]:
        ...
        for i in range(max_atoms):
            for ii in range(i + 1, max_atoms):
                if i < len(vertex[b]) and ii < len(vertex[b]):
                    dec_input[b, idx1, 0], dec_input[b, idx1, 1] = atom_mass[int(vertex[b][i])], atom_mass[int(vertex[b][ii])]
                    dec_input[b, idx1, 2] = idx1 + 1
                    if idx1 == 0: dec_input[b, idx1, 3] = 2
                    elif idx1 == 1: dec_input[b, idx1, 3] = 1
                idx1 += 1
    return src,dec_input
def getInput14(vertex, msp): #transformer without positional encoding
    src = torch.LongTensor([[padding_idx] * k] * len(vertex))  # [batch, k]
    dec_input = torch.LongTensor([[padding_idx] * edge_num*3] * len(vertex))  # [batch, 3*edge_num]
    for b in range(len(vertex)):
        idx = 0
        for j in range(1, len(msp[b]) - 1):
            if msp[b][j - 1] < msp[b][j] and msp[b][j + 1] < msp[b][j]:
        ...
        for i in range(max_atoms):
            for ii in range(i + 1, max_atoms):
                if i < len(vertex[b]) and ii < len(vertex[b]):
                    dec_input[b, idx1 * 3], dec_input[b, idx1 * 3 + 1] = atom_mass[int(vertex[b][i])], atom_mass[int(vertex[b][ii])]
                    if idx1 == 0: dec_input[b, idx1 * 3 +2] = 2
                    elif idx1 == 1: dec_input[b, idx1 * 3 + 2] = 1
                idx1 += 1
    return src,dec_input
...
def train13(model, epoch, num):
    ...
    for batch, i in enumerate(range(0, len(num), batch_size)):
        ...
        labels_graph = mol_adj_data[i:i + seq_len]
        bonds= []
        bonds.append(dec_input[0,0,3])
        bonds.append(dec_input[0,1,3])
        probs = []
        for dd in range(2,edge_num):
            preds = model(src,dec_input[:,:dd],dec_input[:,dd])  # batch, edge_num, 4
            preds_bond = torch.argmax(preds, dim=-1)  # batch edge_num
            while(isValid(preds_bond[0])==0):
                preds[0,preds_bond] = -100000
                preds_bond = torch.argmax(preds, dim=-1)
            bonds.append(preds_bond[0])
            dec_input[:, dd, 3] = preds_bond[0]
            probs.append(preds)
        probs = torch.Tensor(probs)
        optimizer.zero_grad()
        loss = criterion(probs[2:].view(-1, 4), labels[0,2:].view(-1))
        loss.backward()
        optimizer.step()
        total_loss += loss.item()
...
def train31(model,epoch,num):
    ...
    for batch, i in enumerate(range(0, len(num), batch_size)):
        ...
        m = torch.distributions.Categorical(probs) # batch, edge_num, 4
        preds_bond = m.sample()  # edge_num
        log_probs = []
        rewards = []
