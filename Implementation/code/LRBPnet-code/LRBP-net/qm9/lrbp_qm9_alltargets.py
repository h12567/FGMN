...
transform = T.Compose([Complete(), T.Distance(), T.Center(),MyTransform()])#, T.Center(), Complete(),  
...
def test(loader, istest=False):
    ...
    for data in loader:
        data = data.to(device)
        pred = model(data)
        err += ((data.y * std - pred * std).abs() ).sum(dim=0)
        error += ((data.y * std - pred * std).abs() / std ).sum(dim=0)
    
    error = error / len(loader.dataset)
    err = err / len(loader.dataset)
    error_log = torch.log(error)
    if istest: 
        print ('Test error: ')
        print (np.array2string(err.squeeze().cpu().numpy()))
...
if args.test:
    ...
    for epoch in range(1, 21):
        ...
        if best_val_error is None or val_error <= best_val_error:
            ...
            if args.model_save:
                print ("Saving model...")
                torch.save(model.state_dict(),"qm9/models/qm9_alltargets_"+ args.mode +".pth")
