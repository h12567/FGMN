...
class BPNet(nn.Module):
    def __init__(self, num_iters=3, order=3, in_dim=13, rank=128, num_classes=26,mode='CABT'):
    ...
    def forward(self, nodes, edges, edge_types, atoms, atom_edges):#, atom_types):         
        ...
        for itr in range(self.num_iters):
            ...
            for i in range(edges.size(1)):
                ...
                if i == 0:
                    node_msg = (nodes_to_edges[i]) @ msg_x[i]#[edge_types[:,i], numer,:]
                else:
                    node_msg = node_msg + (nodes_to_edges[i] ) @ msg_x[i]#[edge_types[:,i], numer,:]
