...
_urls = {
    'dev': 'https://alchemy.tencent.com/data/dev_v20190730.zip',
    'valid': 'https://alchemy.tencent.com/data/valid_v20190730.zip',
    'test': 'https://alchemy.tencent.com/data/test_v20190730.zip',
}
...
class TencentAlchemyDataset(InMemoryDataset):
    ...
    def __init__(self,
        ...
        #print (self.mode, 'inside')
    @property
    def raw_file_names(self):
        # return [self.mode + '/sdf', self.mode +
        #         '/dev_target.csv'] if self.mode == 'dev' 
        #         elif [    self.mode + '/sdf' , self.mode +
        #         '/valid_target.csv'
        #         ]
        if self.mode == 'dev':
            return [self.mode + '/sdf', self.mode + '/all_targets.csv'] #'/dev_target.csv']
        if self.mode == 'valid':
            return [self.mode + '/sdf', self.mode + '/all_targets.csv'] #'/valid_target.csv']
        if self.mode == 'test':
            return [self.mode + '/sdf', self.mode + '/all_targets.csv'] #'/test_target.csv']
    ...
    def sdf_graph_reader(self, sdf_file):
        ...
        l = torch.FloatTensor(self.target.loc[int(sdf_file.stem)].tolist()).unsqueeze(0) \
                if (self.mode == 'dev' or self.mode == 'valid' or self.mode == 'test') else torch.LongTensor([int(sdf_file.stem)])
    ...
    def process(self):
        if (self.mode == 'dev' or self.mode == 'valid' or self.mode == 'test'):
