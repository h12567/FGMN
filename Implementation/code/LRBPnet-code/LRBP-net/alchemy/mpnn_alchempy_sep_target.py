...
transform = T.Compose([Complete(), T.Distance(norm=False), MyTransform()]) #Complete(), 
...
def test(loader, istest=False):
    ...
    for data in loader:
        data = data.to(device)
        pred = model(data)
        error += ((data.y - pred ).abs()).sum().item()
...
if args.test:
    ...
    test_error, _ = test(test_loader, istest=True)
    ...
    for epoch in range(1, 201):
        ...
        if best_val_error is None or val_error <= best_val_error:
            ...
            if args.model_save:
                save_file= "alchemy/models/Alchemy_MPNN_target_"+args.target+".pth"
    ...
    print("results mean and std: ",results.mean(), results.std())

    time_log = np.array(time_log)
    print("Time mean and std: ",time_log.mean(), time_log.std())    
