...
class FGMNDataset(InMemoryDataset):
    ...
    def get_edge_nodes(self, mol_adj_arr, num_atoms, node_features, node_labels, edge_idx):
    ...
    def get_msp_nodes(self, msp_arr, k, num_atoms, node_features, node_labels, edge_idx):
    ...
    def process(self):
        ...
        for i in range(atom_arr_all.shape[0]):
            ...
            node_features, node_labels, edge_idx = [], [], []
            self.get_atom_nodes(atom_arr, node_features, node_labels)
            self.get_edge_nodes(mol_adj_arr, len(atom_arr), node_features, node_labels, edge_idx)
            self.get_msp_nodes(msp_arr, self.NUM_MSP_PEAKS, len(atom_arr), node_features, node_labels, edge_idx)
            ...
            data = Data(x=node_features, edge_index=edge_idx, y=node_labels)
...
dataset = FGMNDataset(path).shuffle()
a = 1
